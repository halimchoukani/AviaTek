...
export default function RequestTraining() {
    ...
    const [user, setUser] = useState<PilotDocument | null>(null);
    ...
    const [category, setCategory] = useState<string>("Recurrency");
    ...
    useEffect(() => {
        const init = async () => {
            try {
                const [currentUser, planesData, simulatorsData] = await Promise.all([
                    getCurrentUser() as unknown as Promise<PilotDocument>,
                    getPlanes(),
                    getSimulators()
                ]);
                ...
                // Fetch Academy info
                const academyId = currentUser.academy || currentUser.prefs?.academyId;
                if (academyId) {
                    const academyData = await getAcademyById(academyId) as unknown as AcademyDocument;
                    setAcademy(academyData);
                } else {
                    const academyData = await getAcademyForPilot();
                    if (academyData) setAcademy(academyData);
                }

                // Set initial selection if data exists
                if (planesData.length > 0) {
                    setSelectedEquipment(planesData[0].$id);
                }
            } catch (error) {
                console.error("Error initializing request screen:", error);
            } finally {
  ...
  try {
const combinedStartDate = `${date} ${preferredTimes}`;

    await sendRequest({
      pilotId: user.$id,
      academyId:
        academy?.$id || user.academy || user.prefs?.academyId || "",
    ...
    if (fetchingUser || fetchingEquipment) {
        return (
